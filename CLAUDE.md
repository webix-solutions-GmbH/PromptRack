...
```bash
npm run dev                          # starts the dev postgres, migrates, serves http://localhost:3000
...
Git: branch is `master` (not main); `origin` is GitHub (`philphilphil/modelfit`), and the former Azure DevOps remote is kept as `azure`. Commits so far are milestone-sized with imperative messages.

## What this is

**modelfit** answers two questions for a consultancy that sells AI solutions to businesses: **which model is good enough for this customer's actual job**, and **what hardware that takes**. Not a leaderboard score — the customer's real work, loaded in as prompt suites: an invoice-processing agent, document and data extraction, structured extraction from business correspondence, MCP tool calls against the company's own RAG. The suites are the specification of the job, and the app's answer is a fitness verdict per model on *those* tasks, never a general ranking.

The second question is sizing, and it is why every result names the **machine** that produced it (an endpoint plus free-text hardware notes): if a small model does the job, a DGX Spark or even a Mac Mini is enough — but that has to be measured, and TTFT/duration/tok-s per machine is the evidence. Endpoints are anything OpenAI-compatible, so this is **not local-only**: Ollama / LM Studio / vLLM on your own boxes *and* hosted frontier APIs, side by side in one matrix. The common outcome is a **mixed deployment** — most workloads self-hosted, the hard ones routed to a frontier model — and the app exists to find where that line falls. Workspaces are per **customer engagement**, which is the whole reason they exist: one engagement's machines, prompts and runs stay out of another's.

Mechanically, multi-user and multi-workspace: define test prompts (grouped, optionally with expected output — the rubric), run them sequentially against a machine's endpoint, measure TTFT/duration/tokens/tok-s, rate results good/meh/bad manually, compare in a matrix by model or by run. A prompt can also be a **tool test**: offer the model a set of functions and either record what it wanted to call, or really execute the calls through an MCP server and loop until it answers — which is how an invoice agent or a RAG-backed assistant gets evaluated as the agent it will actually be, not as a chat completion.
...
- **basePath** (`next.config.ts`, constant `BASE_PATH` in `src/lib/base-path.ts`, currently `''` — the app serves at the root): `next/link` and the router prefix automatically, but raw client `fetch()` calls to our own API routes MUST go through `apiPath()` from `src/lib/base-path.ts`. Restoring a sub-path deployment is a one-line change to `BASE_PATH`.
...
- **The example suite is a document, not a seeder**: `docs/example-suite/` is built into a
  workspace over MCP, so it lands wherever the caller's `X-Customer` points and needs no
  per-workspace idempotency ledger at all. See *The example suite* below.
...
- **basePath and better-auth.** Next strips whatever basePath is configured from `request.url` before a route handler sees it, but better-auth routes on `new URL(ctx.baseURL).pathname` and builds every absolute URL (the OIDC `redirect_uri` above all) from the same value — so the catch-all handler puts the basePath *back* on the request before delegating, and both `baseURL` and `basePath` carry it (`AUTH_BASE_PATH` in `base-path.ts`, shared with the browser client). With `BASE_PATH` currently `''` the catch-all's re-prepend is a no-op (every pathname already starts with `/`), and both auth URLs resolve to `/api/auth/...`. Getting this wrong is a silent 404 from better-auth's own router, not an error.
...
`POST /api/mcp` lets an agent author the evaluation from outside: push another project's real system prompts and prompts in, start a run, read the measurements back. The point is that the interesting test cases already exist in other repos — a customer's own agent repository is where the job is defined.
...
`docs/example-suite/` is the standard suite — 3 manual toolsets, 38 prompts in 4 groups — written as **documentation an agent executes**, not as a script. It replaced `scripts/seed-prompts.mjs` (`npm run db:seed`) and its `__app_seeds` ledger, both deleted, the table dropped in `drizzle/0006_drop_app_seeds.sql`.

The reason is that a seeder and this app disagree about who owns the data. A script had to reimplement scoping (`SEED_CUSTOMER`), idempotency (a ledger keyed by workspace so "seeded once, then deleted, stays deleted" survived), and every validation `createRunRecord` already does — 1333 lines of it, none of which the app itself needed. Handing the same content to an agent over MCP moves all four concerns to code that already exists: the workspace comes from `X-Customer`, `create_prompt_group` is already name-idempotent, and `assertToolConfig` already refuses what the prompt editor refuses. What is left is prose a human can also read, edit for a specific customer, or hand over as a starting point — which is what people actually did with the seed script.

The split inside it is forced by the app's own rule: **toolsets are not writable over MCP** (they hold an MCP URL and headers, i.e. credentials), so `toolsets.md` is instructions for a human in the UI and the four group files are for the agent. Six of the 38 prompts are tool tests and need step one done first; the other 32 do not.

Canned tool responses are written to stay correct *whatever arguments the model passes* — `convert_currency` returns a rate rather than a converted amount, so the response can never contradict the call and the model still has to do the arithmetic.
...
`npm run test:integration` (`vitest.integration.config.ts`, `tests/integration/**`) runs against the scratch Postgres described above, with `fileParallelism: false` because the suites share one database and `tests/integration/setup.ts` truncates every table between tests. It covers what only a real database can show: the FK cascade/set-null actions and `Date`/`boolean`/float8 round-tripping (`schema.test.ts`), the snapshot invariant and the `createRunRecord` rollback (`run-create.test.ts`), the advisory-lock claim (`run-lock.test.ts`), and cross-workspace isolation (`workspaces.test.ts` — the two snapshot/compare leak paths, foreign ids over MCP, the delete guard). `seed.test.ts` went with the seed script; the Unicode-Tags payload it guarded is now prose in `docs/example-suite/`, reconstructed on demand rather than shipped as invisible characters, so there is no round-trip left to protect.

Everything else is verified against the dev server + the mocks:

- **Mock LLM** — register a machine with base_url `http://localhost:3000/api/mock-llm`. User messages containing `TRIGGER_ERROR` → 500, `TRIGGER_SLOW` → 2s TTFT delay, `TRIGGER_TOOL_LOOP` → never stops calling tools (exercises `max_turns`). When the request carries `tools` and no tool result yet, it streams a tool call for the first tool with arguments synthesized from its schema, split across chunks; once a tool result is present it answers in text quoting it.
- **Mock MCP** — an MCP toolset pointing at `http://localhost:3000/api/mock-mcp` serves `echo_upper` and `add_numbers`. `?hide=<tool>` drops one from `tools/list` (verifies discovery disables rather than deletes), `?fail=1` makes every call return `isError`.
